...
static const char ONBOARD_HTML[] =
    ...
    "<input id='mqtt_port' type='number' placeholder='1883'>"
    "<label>Image Backend URL</label>"
    "<input id='upload_backend_url' placeholder='http://192.168.1.100:8080'>"
    "<label>Image Upload Path</label>"
    "<input id='upload_api_path' placeholder='/save'>"
    "</div></div>"
    ...
    "'mqtt_broker','mqtt_port','upload_backend_url','upload_api_path'];"
    "var data={};"
    ...
    "    window.ReactNativeWebView.postMessage(JSON.stringify({type:'CONFIG_SUCCESS',mac:resp.mac||data.mac,mqtt:resp.mqtt||data.mqtt_broker||''}));"
    "  }"
    "})"
